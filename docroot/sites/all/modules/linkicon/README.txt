...
Intended for site builders and themers who want to gain control over social 
links appearance by design. It doesn't add icons you want to override/remove 
once installed. If you want icons included, sorry to disappoint :p

Drupal supports unlimited values, the limitation is available icon fonts.


Features:
========
- Predefine allowed titles.
...
- Optional simple stylings: pure CSS tooltip, square, rounded and base colors, 
  or disable it from outputting CSS, if you care to DIY. It's just starter anyway.
- Options to display or hide text, feed icon fonts CSS file, and a few other.
- And of course, link module own virtues as a field.


Extending:
==========
- Use Views to place it as blocks , contextual filter by node/user ID from URL.
- For sitewide social links, an idea is to put the links into admin account field,
  and use Views to make them as sitewide block.
...
You don't need this module if you only have a simple need with a fixed set icons.
...
- At admin/config/people/accounts/fields, or Content types -- click "Manage fields". 
...
- At admin/config/people/accounts/fields, or Content types -- click "Manage display". 
...
- Click the "Configure" icon to have some extra options. There is option to hide
  text so to display the icon only, option to disable module from outputting CSS,
...
It adds sitewide and per user links that link to various social networking sites.
The links reside in two blocks.
...
Credits
=======
Gaus Surahman (gausarts) http://gausarts.com
