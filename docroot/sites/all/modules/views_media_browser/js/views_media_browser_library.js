  ...
  attach: function (context, settings) {
    ...
    prepLibrary = function (event, ui) {
      ...
      if (ui.tab.hash === '#media-tab-library') {
        ...
        params['display_format'] = $('#edit-display-format').attr('value');
        library.start($(ui.panel), params);
        $('#scrollbox').bind('scroll', library, library.scrollUpdater);
        $('#scrollbox').removeClass();
        $('#scrollbox').addClass($('#edit-display-format').attr('value'));
      }
    };
    $('#media-browser-tabset').bind('tabsshow', prepLibrary);
  }
};
...
Drupal.media.browser.library.prototype.render = function (renderElement) {
  ...
  while (this.cursor < this.mediaFiles.length) {
    ...
    $(mediaList).append($(mediaFile.preview).addClass('media-item-' + mediaFile.fid).bind('click', data, this.clickFunction));
  }

  $(mediaList).find('a').each(function() {
    $(this).click(function() {
