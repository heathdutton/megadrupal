...
function sideMainClick(page, top) {
	...
	sb.page = page;
	sb.topTab = top;
	var x = new XHR({url: fix_page_url(sb.page), onSuccess: finishTabLoad}).send();
}
...
function browseNav(start, view) {
	sb.s = start;
	sb.view = view;
	var x = new XHR({url: fix_page_url("browse.inc"), onSuccess: finishTabLoad}).send("s=" + f(sb.s) + "&view=" + f(sb.view) + "&sortKey=" + f(sb.sortKey) + "&sortDir=" + f(sb.sortDir));
}
...
function editSelectedRows() {
	...
	if (f(grid)) {
		...
		if (count > 0) {
			if (sb.page == "structure.inc")
				var loadPage = "editcolumn.inc";
			else if (sb.page == "users.inc")
				var loadPage = "edituser.inc";
			else
				var loadPage = "edit.inc";
			
			editParts = editParts.substring(0, editParts.length - 2);
			sb.topTabs[sb.topTabSet].addTab("Edit", loadPage, true);
			sb.page = loadPage;
			var x = new XHR({url: fix_page_url(loadPage), onSuccess: finishTabLoad}).send("editParts=" + editParts);
		}
	}
}
...
function saveEdit(formId) {
	...
	var x = new XHR({url: fix_page_url("ajaxsaveedit.inc?queryPart=" + queryPart + "&form=" + formId), onSuccess: updateAfterEdit}).send(form.toQueryString());
}
...
function saveColumnEdit(formId) {
	...
	var x = new XHR({url: fix_page_url("ajaxsavecolumnedit.inc?form=" + formId), onSuccess: updateAfterEdit}).send("runQuery=" + columnQuery);
}

function saveUserEdit(formId) {
	var form = $(formId);
	var x = new XHR({url: fix_page_url("ajaxsaveuseredit.inc?form=" + formId + "&user=" + form.get("querypart")), onSuccess: updateAfterEdit}).send(form.toQueryString());
}
...
function hideUpdateMessages() {
	...
	if (edits.length == 0) {
		...
		if (sb.page == "edituser.inc" || sb.page == "editcolumn.inc")
			topTabLoad(1);
		else
			topTabLoad(0);
	} else {
...
function optimizeSelectedTables() {
	...
	if (f(lc)) {
		...
		if (optimizeQuery) {	
			var x = new XHR({url: fix_page_url(sb.page), onSuccess: finishTabLoad}).send("runQuery=" + optimizeQuery);
		}
	}
}
...
function executeQuery() {
	...
	var x = new XHR({url: fix_page_url("query.inc"), onSuccess: finishTabLoad}).send("query=" + query);
}

function loadNewSort(key, direction) {
	sb.sortKey = key;
	sb.sortDir = direction;
	var x = new XHR({url: fix_page_url("browse.inc"), onSuccess: finishTabLoad}).send("view=" + sb.view + "&s=" + sb.s + "&sortKey=" + sb.sortKey + "&sortDir=" + sb.sortDir);
}
...
function confirmEmptyTable() {
	if (f(sb.table)) {
		...
		showDialog(gettext("Confirm"),
			...
			"var x = new XHR({url: fix_page_url(\"ajaxquery.inc\"), onSuccess: emptyTableCallback}).send(\"query=" + encodeURIComponent(emptyQuery) + "&silent=1\")"
		);
	}
}
...
function confirmDropTable() {
	...
	if (f(table)) {
		...
		showDialog(gettext("Confirm"),
			...
			"var x = new XHR({url: fix_page_url(\"ajaxquery.inc\"})).send(\"query=" + dropQuery + "&silent=1\"); $(getSubMenuId(sb.db, sb.table)).dispose(); databaseLoad(sb.db); recalculateSubmenuHeight(\"" + toRecalculate + "\");"
		);
	}
}
...
function optimizeTable() {
	if (sb.table) {
		...
		var x = new XHR({url: fix_page_url("ajaxquery.inc"), onSuccess: function(){ sb.loadPage() } }).send("query=" + optimizeQuery + "&silent=1");
	}
}
...
function confirmDropDatabase() {
	...
	if (f(db)) {
		...
		showDialog(gettext("Confirm"),
			...
			"var x = new XHR({url: fix_page_url(\"ajaxquery.inc\"})).send(\"query=" + dropQuery + "&silent=1\"); $(getSubMenuId(sb.db, sb.table)).dispose(); sideMainClick(\"home.inc\",\"hometab\");"
		);
	}
}
...
function editTable() {
	...
	if (f(runQuery) != "") {
		$('RENAME').blur();
		var x = new XHR({url: fix_page_url("ajaxquery.inc"), onSuccess: editTableCallback}).send("query=" + runQuery + "&silent=1");
	}
...
function editDatabase() {
	...
	if (f(newCharset) != "") {
		...
		var x = new XHR({url: fix_page_url("ajaxquery.inc"), onSuccess: editDatabaseCallback}).send("query=" + runQuery + "&silent=1");
	}
...
function sizePage() {
	...
	if (f(sb.grid) && (sb.page == "browse.inc" || sb.page == "query.inc")) {
		if (sb.page == "browse.inc")
			var gridHeight = windowInnerHeight - 111;
		else if (sb.page == "query.inc")
...
function getSubMenuId(db, table) {	
	if (f(db) && f(table))
		...
		return "sideusers";
	else if (sb.page == "import.inc")
		return "sideimport";
	else if (sb.page == "export.inc")
		return "sideexport";
	else
		return "sidehome";
}
...
function createDatabase() {
	...
	if (f(dbName)) {
		...
		var x = new XHR({url: fix_page_url("ajaxquery.inc"), onSuccess: createDatabaseCallback}).send("query=" + createQuery + "&silent=1");
	}
...
function createTable() {
	...
	if (f(tableName) && fields.length > 0) {
		...
		var x = new XHR({url: fix_page_url("ajaxcreatetable.inc"), onSuccess: createTableCallback}).send("table=" + tableName + "&query=" + createQuery);
	}
	else if (!(f(tableName))) {
...
function submitAddColumn() {
	...
	var x = new XHR({url: fix_page_url(sb.page), onSuccess: finishTabLoad}).send("runQuery=" + columnQuery);
}
...
function refreshRowCount() {
	if (f(sb.db) && f(sb.table)) {
		...
		var x = new XHR({url: fix_page_url("ajaxquery.inc"), onSuccess: updateRowCount, showLoader: false}).send("query=" + countQuery);
	}
}
...
function switchLanguage() {
	...
	if (lang != defaultLang) {
		var co = Cookie.write("sb_lang", lang, {duration: 60, path: '/sqlbuddy'});
...
function switchTheme() {
	...
	if (theme != defaultTheme) {
		var co = Cookie.write("sb_theme", theme, {duration: 60, path: '/sqlbuddy'});
...
function autoExpandTextareas() {
	...
	if (taList.length > 0 && sb.page != "export.inc") {
