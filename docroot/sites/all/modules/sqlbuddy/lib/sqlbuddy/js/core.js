...
function fix_page_url(url) {
    var m = url.match(/([^.]+)[.]inc(.*)$/);
    return '/sqlbuddy/' + m[1] + m[2];
}
...
function Page() {
	...
	if (showUsersMenu) {
		this.topTabs[0].addTab(gettext("Home"), "home.inc").addTab(gettext("Users"), "users.inc").addTab(gettext("Query"), "query.inc").addTab(gettext("Import"), "import.inc").addTab(gettext("Export"), "export.inc");
	} else {
		this.topTabs[0].addTab(gettext("Home"), "home.inc").addTab(gettext("Query"), "query.inc").addTab(gettext("Import"), "import.inc").addTab(gettext("Export"), "export.inc");
	}
	
	this.topTabs[1].addTab(gettext("Overview"), "dboverview.inc").addTab(gettext("Query"), "query.inc").addTab(gettext("Import"), "import.inc").addTab(gettext("Export"), "export.inc");
	this.topTabs[2].addTab(gettext("Browse"), "browse.inc").addTab(gettext("Structure"), "structure.inc").addTab(gettext("Insert"), "insert.inc").addTab(gettext("Query"), "query.inc").addTab(gettext("Import"), "import.inc").addTab(gettext("Export"), "export.inc");
	
}

Page.prototype.loadPage = function() {
	if (this.page == "browse.inc" && parseInt(this.tableRowCounts[this.db + "_" + this.table]) == 0) {
		this.page = "structure.inc";
		this.topTab = 1;
	}
	...
	if (this.page == "editcolumn.inc") {
		this.page = "structure.inc";
		this.topTab = 1;
	} else if (this.page == "edituser.inc") {
		this.page = "users.inc";
		this.topTab = 1;
	} else if (this.page == "edit.inc") {
		this.page = "browse.inc";
		this.topTab = 0;
	}
	
	clearPanesOnLoad = true;
	var x = new XHR({url: fix_page_url(this.page), onSuccess: finishTabLoad}).send(pageUrl);
}
...
Page.prototype.refreshTopTabSet = function(setNum) {
	...
	for (var i=0; i<loopStop; i++) {	
		...
		if (currentTab.url == "browse.inc" && browseIsActive == false)
			tabLiObj.addClass('deactivated');
		tabAObj = new Element('a');
		tabAObj.appendText(currentTab.title);
		if (currentTab.url == "browse.inc" && f(rowCount) != "") {
			tabACount = new Element("span");
			tabACount.addClass("rowcount");
			tabACount.appendText("(" + rowCount + ")");
			tabAObj.appendChild(tabACount);
		}
		if (tabId != this.topTab && !(currentTab.url == "browse.inc" && browseIsActive == false) && !currentTab.temp)
			tabAObj.onclick = topTabClicked;
		if (!(currentTab.url == "browse.inc" && browseIsActive == false) && !currentTab.temp) {
...
Page.prototype.preload = function() {
	...
	for (var i=0; i<images.length; i++) {
		pre = new Image();
		pre.src = "/sqlbuddy/images/" + images[i];
	}
}
...
Page.prototype.loadHash = function() {
	...
	for (var i=0; i<pairs.length; i++) {
		...
		if (key == "page"){ this.page = value + ".inc" }
	...
	if (f(grid)) {
		...
		if (buildList) {
			...
			showDialog(gettext("Confirm"),
				prompter,
				"var x = new XHR({url: fix_page_url(\"" + sb.page + "\"), onSuccess: finishTabLoad}).send(\"" + buildUrl + "\");"
			);
		}
	}
}
...
function submitForm(formId) {
	...
	var x = new XHR({url: fix_page_url(action), onSuccess: finishTabLoad}).send(theForm.toQueryString());
}
...
function fullTextWindow(rowId) {
	...
	var x = new XHR({
		url: fix_page_url("ajaxfulltext.inc"),
...
function createWindow(title, content, options) {
	...
	var windowHeaderContent = '<table cellspacing="0" width="100%"><tr><td class="headertl"></td><td class="headercenter"><p><img class="fulltextimage" src="/sqlbuddy/images/window-close.png" align="right" onclick="closeWindow(\'' + windowId + '\')" />' + title + '</p></td><td class="headertr"></td></tr></table>';
	...
	if (!(options.isDialog || options.isDismissible)) {
		innerContent += '<div class="resizeHandle"><img src="/sqlbuddy/images/window-resize.png" id="resize' + windowId + '"></div>';
	}
...
function runKeyboardShortcuts(e) {
	...
	if (!((event.target.nodeName == "INPUT" && (event.target.type == "text" || event.target.type == "password")) || (event.target.nodeName == "TEXTAREA") || event.meta || event.control)) {
		if (event.key == "a") {
			...
			if (sb.page == "browse.inc" || sb.page == "structure.inc" || sb.page == "users.inc")
				editSelectedRows();
		} else if (event.key == "d") {
			if (sb.page == "structure.inc")
				...
				deleteSelectedUsers();
			else if (sb.page == "browse.inc")
				deleteSelectedRows();
			else if (sb.page == "dboverview.inc")
				dropSelectedTables();
		} else if (event.key == "r") {
			sb.loadPage();
		} else if (event.key == "f" && sb.page == "browse.inc") {
			if ($('firstNav'))
				eval($('firstNav').get("onclick"));
		} else if (event.key == "g" && sb.page == "browse.inc") {
			if ($('prevNav'))
				eval($('prevNav').get("onclick"));
		} else if (event.key == "h" && sb.page == "browse.inc") {
			if ($('nextNav'))
				eval($('nextNav').get("onclick"));
		} else if (event.key == "l" && sb.page == "browse.inc") {
			if ($('lastNav'))
				eval($('lastNav').get("onclick"));
		} else if (event.key == "q") {
			var tabId = 0;
			while (sb.getTabUrl(tabId) != "query.inc") {
				tabId++;
			}
			topTabLoad(tabId);
		...
		} else if (event.key == "o" && sb.page == "dboverview.inc") {
