...
abstract class AbstractMessage implements MessageInterface {
  ...
  public function setBody($body = NULL) {
    ...
    if (isset($body->ResponseMessage)) {
      $messages = explode(',', $body->ResponseMessage);
      foreach ($messages as $message) {
        $this->body->status[$message] = ResponseCode::getMessage($message);
      }
    }

    if (isset($body->Errors)) {
      $errors = explode(',', $body->Errors);
      foreach ($errors as $code) {
        $this->body->ErrorPhases[$code] = ResponseCode::getMessage($code);
      }
    }
