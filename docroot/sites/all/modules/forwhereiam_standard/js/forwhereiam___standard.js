    ...
    attach: function (context, settings) {
      ...
      $('#' + block_id, context).once('fwiastandardbehaviour', function () {
              ...
              $.ajax({
                ...
                complete: function(xhr) {
                  ...
                  if (json && json.data && json.data.error) {
                    ...
                    // If trigger was on widget and not popup, then perform a search
                    if (el.id=='fwia---standard-geolocation-link') 
		    {
		      ...
		      $('#' + block_id + ' .fwia---standard-location').val(json.data.postcode);
		      $('#fwia-popup-location .fwia---standard-location').val(json.data.postcode);
		    }
                  }
                }
              });
        ...
        if (geolocate && window.navigator.geolocation) {
          ...
          $('.fwia---standard-geolocation', context).live('click', function() { _findMe(this); return false; });
        }
        else {
          geolocation.hide();
        }
