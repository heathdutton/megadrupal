...
/**
 * @file
 * Documents hooks provided by the alignet modules.
 */
 ...
 *
 * When the alignet module receives an Instant Payment Notification (IPN) from
 * alignet, it performs some basic validation, allows the payment method detected
 ...
 * parent_txn_id and auth_id values. See the alignet WPS IPN process callback for
 ...
 * @param $ipn
 *   The IPN array received from alignet after it has been saved, including the
 *   additional ipn_id, order_id, and transaction_id values.
 *
 * @see commerce_alignet_wps_alignet_ipn_process()
 */
function hook_commerce_alignet_ipn_process($order, $payment_method, $ipn) {
  // No example.
}

/**
 * Allows modules to alter the name-value pair array for a alignet API request
 * before it is submitted.
 *
 * Currently invoked for alignet Payments Pro and Express Checkout. Modules
 ...
 */
function hook_commerce_alignet_api_request_alter(&$nvp, $order, $payment_method) {
  // No example.
}
