 ...
 * use its own uri callback that checks commerce_product_key_uri() for a return
 ...
 *
 * @see commerce_product_key_uri()
 * @see entity_uri()
 */
function hook_commerce_product_key_uri($product_key) {
  // If the product has a display context, use it entity_uri().
  if (!empty($product_key->display_context)) {
    return entity_uri($product_key->display_context['entity_type'], $product_key->display_context['entity']);
  }
}
 ...
 */
function hook_commerce_product_key_presave(&$product_key) {
  // No example.
}
 ...
 *
 * @see commerce_product_key_reference_commerce_product_key_can_delete()
 */
function hook_commerce_product_key_can_delete($product_key) {
  ...
  $query
    ...
    ->fieldCondition('product', 'product_key_id', $product_key->product_key_id, '=')
    ->count();
