/**
 * @file media_youku/js/media_youku.js
 */

(function($) {

	Drupal.media_youku = {};
	Drupal.behaviors.media_youku = {
		attach : function(context, settings) {
			...
			if (!FlashDetect.installed && !html5) {
				$('.media-youku-preview-wrapper').each(
						Drupal.media_youku.needFlash);
			}
		}
	};

	Drupal.media_youku.needFlash = function() {
		var id = $(this).attr('id');
		var wrapper = $('.media-youku-preview-wrapper');
		var hw = Drupal.settings.media_youku[id].height
				/ Drupal.settings.media_youku[id].width;
		wrapper
	...
	Drupal.media_youku.insertEmbed = function(embed_id) {
		var videoWrapper = $('#' + embed_id + '.media-youku-preview-wrapper');
		var settings = Drupal.settings.media_youku[embed_id];

		// Calculate the ratio of the dimensions of the embed.
		settings.hw = settings.height / settings.width;

		// Replace the object embed with YouKu's iframe. This isn't done by
		// the
		// theme function because YouKu doesn't have a no-JS or no-Flash
		// fallback.
		var video = $('<iframe class="youku-player" type="text/html" frameborder="0"></iframe>');
		var src = 'http://www.youku.com/embed/' + settings.video_id;

		// Allow other modules to modify the video settings.
		settings.options.wmode = 'opaque';
		$(window).trigger('media_youku_load', settings);

		// Merge YouKu options (such as autoplay) into the source URL.
		...
		$(window).bind('resize', Drupal.media_youku.resizeEmbeds);
		...
		if (!$('.lightbox-stack').length) {
			setTimeout(Drupal.media_youku.resizeEmbeds, 1);
		}
	};

	Drupal.media_youku.resizeEmbeds = function() {
		$('.media-youku-preview-wrapper').each(
				Drupal.media_youku.resizeEmbed);
	};

	Drupal.media_youku.resizeEmbed = function() {
		var context = $(this).parent();
		var video = $(this).children(':first-child');
		var hw = Drupal.settings.media_youku[$(this).attr('id')].hw;
		// Change the height of the wrapper that was given a fixed height by the
		// YouKu theming function.
