/**
 * @file Create the 'YouKu' tab for the WYSIWYG plugins.
 */

// (function ($) {
// namespace('Drupal.media.browser.plugin');
//
// Drupal.media.browser.plugin.media_youku = function(mediaBrowser, options) {
// return {
// init: function() {
// tabset = mediaBrowser.getTabset();
// tabset.tabs('add', '#media_youku', 'YouKu');
// mediaBrowser.listen('tabs.show', function (e, id) {
// if (id == 'media_youku') {
// // We only need to set this once.
// // We probably could set it upon load.
// if (mediaBrowser.getActivePanel().html() == '') {
// mediaBrowser.getActivePanel().html(options.media_youku);
// }
// }
// });
// }
...
// //@todo: needs a review.
// Drupal.media.browser.register('media_youku',
// Drupal.media.browser.plugin.media_youku, {});
// })(jQuery);
(function($) {
	namespace('media.browser.plugin');

	Drupal.media.browser.plugin.youku_library = function(mediaBrowser,
			options) {
		...
		return {
			...
			init : function() {
				tabset = mediaBrowser.getTabset();
				tabset.tabs('add', '#youku_library', 'YouKu');
				var that = this;
				mediaBrowser.listen('tabs.show', function(e, id) {
					if (id == 'youku_library') {
						// This is kinda rough, I'm not sure who should delegate
						// what here.
			...
			getMedia : function() {
				...
				jQuery.get(callback, params, function(data, status) {
			...
			render : function() {
				...
				if (this.mediaFiles.length < 1) {
					jQuery(
							'<div id="media-empty-message" class="media-empty-message"></div>')
							.appendTo(this.browser).html(this.emptyMessage);
					return;
				}
					...
					var imgLink = jQuery('<a href="#"></a>').appendTo(listItem)
							.html(mediaFile.preview).bind(
									...
									function(e) {
										...
										$('div.media-thumbnail img', $(this))
												.addClass('selected');
										mediaBrowser.notify('mediaSelected', {
											mediaFiles : [ e.data ]
										});
				...
				jQuery('<div id="media-pager" class="media-pager"></div>')
						.appendTo(this.browser).html(this.pager);
			}
		};
	};

	Drupal.media.browser.register('youku_library',
			Drupal.media.browser.plugin.youku_library);
})(jQuery);
