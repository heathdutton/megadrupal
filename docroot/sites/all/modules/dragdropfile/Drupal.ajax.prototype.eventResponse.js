  ...
  try {
    if (ajax.form) {
      ...
      if ( 'FormData' in window  && jQuery(element).hasClass('i-am-dragdropfile') ) {
        ...
        // This is weird. Override jQuery's transport finder to get access to the native xhr object...
        if (ajax.options.onProgress) {
          ajax.options.xhr = function() {
            var xhr = jQuery.ajaxSettings.xhr();
            (xhr.upload || xhr).addEventListener('progress', function(e) {
              e.total || (e.total = e.totalSize);
              e.done = e.loaded || e.position;
              ajax.options.onProgress.call(ajax, e, this);
            });
            return xhr;
          };
        }
