...
- Main settings
  Content types: Enable/disable the content types you want Flexi
  Access to manage (e.g. Article). There is also a panel to manage
  per-type settings for automatic ACL creation.  Enabling automatic
  ACL creation for a content type will make access to newly created
  nodes restricted by default. Enabling this will create an empty ACL
  for every new node created, thereby restricting access to the node
  by all users until they're added to the node's ACL. This does not
  affect existing nodes.

- Anonymous user
  Allow administrator to store a string in the database as the name
  for the Anonymous user (user #0).
...
to see the node.  Creating an ACL and not naming anyone will hide the
node from all users.
...
Administrators viewing a user profile will see a tab named Flexi
Access as part of that profile.  This tab is to manage the ACL for
...
2. What Flexi Access eventually will hav to offer is a richer user
   interface that, among other -things, allows the administrator to
   create and maintain predefined user lists for ACL purposes.

   What is needed is an UI to create a user group to contain
...
3. JavaScript to warn user if moving away from page without commiting.
