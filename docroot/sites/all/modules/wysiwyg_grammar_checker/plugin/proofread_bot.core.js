...
var EXPORTED_SYMBOLS = ['proofread_botCore'];

function proofread_botCore() {
	/* these are the categories of errors proofread_bot should ignore */
	...
	/* these are the phrases proofread_bot should ignore */
	this.ignore_strings = {};
...
proofread_botCore.prototype.getLang = function(key, defaultk) {
...
proofread_botCore.prototype.addI18n = function(localizations) {
...
proofread_botCore.prototype.setIgnoreStrings = function(string) {
	var parent = this;
...
proofread_botCore.prototype.showTypes = function(string) {
...
proofread_botCore.prototype.makeError = function(error_s, tokens, type, seps, pre) {        
...
proofread_botCore.prototype.addToErrorStructure = function(errors, list, type, seps) {
	var parent = this;                  
...
proofread_botCore.prototype.buildErrorStructure = function(spellingList, enrichmentList, grammarList) {
...
proofread_botCore.prototype._getSeparators = function() {
	var re = '', i;
	var str = '"s#$%&()*+/<=>@[\]^_{|}';
...
proofread_botCore.prototype.processXML = function(responseXML) {
...
proofread_botCore.prototype.findSuggestion = function(element) {
...
TokenIterator.prototype.next = function() {
	...
	/* strip single quotes from token, proofread_bot does this when presenting errors */
	if (current != "") {
		if (current[0] == "'")
 ...
 */
proofread_botCore.prototype.markMyWords = function(container_nodes, errors) { 
	//console.log(errors);          
	...
	this.map(nl, function(n) {
		...
		if (n.nodeType == 3) {
			...
			//var tokens = n.nodeValue.split(/\b/);
			//console.log(tokens);
			var previous = "";
			...
			while (iterator.hasNext()) {
				...
				if (current != undefined && current.pretoks != undefined) {
					...
					var checkErrors = function(error) {
						//console.log("testing:" + error.regexp);
...
proofread_botCore.prototype._walk = function(elements, f) {
	var i;
...
proofread_botCore.prototype.removeWords = function(node, w) {   
	var count = 0;
	var parent = this;
...
proofread_botCore.prototype.isEmptySpan = function(node) {
...
proofread_botCore.prototype.isMarkedNode = function(node) {
 ...
 */
proofread_botCore.prototype.applySuggestion = function(element, suggestion) {
