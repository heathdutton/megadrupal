...
Drupal.openlayers.addBehavior('openlayers_behavior_fp', function (context, options) {
  ...
  if (layers.length < 1) {
    ...
    if (options.popupOptions.list.popupClass.length > 0) {
      popupOptions = {popupOptions: options.popupOptions};
      // Show single item as single list item.
      popupOptions.popupOptions.single = null;
    }

  var fpControl = new OpenLayers.Control.FeaturePopups(popupOptions);
