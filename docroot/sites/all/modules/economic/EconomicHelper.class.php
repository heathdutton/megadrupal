...
class EconomicHelper {
    ...
    try {
      $this->client = new SoapClient($this->wsdl, array(
        ...
        'stream_context' => stream_context_create(array(
          'http' => array(
            'header' => 'X-EconomicAppIdentifier: Drupal Commerce economic (https://www.drupal.org/project/economic)',
          ),
        )),
      ));
   ...
   *
   * @return bool
   *   Return positive based header accepted or not.
   */
  public function verifyHeader() {
    try {
      ...
      $test = $this->client->Verify_XEconomicAppIdentifier();

      if ($test->Verify_XEconomicAppIdentifierResult == TRUE) {
        return TRUE;
      }
