...
class openid_profile_link {
  ...
  function convert_to_ax($value, $count = 1) {
    if ($count == 1 && !empty($value[LANGUAGE_NONE][0]['url'])) {
      return $value[LANGUAGE_NONE][0]['url'];
    }
    else {
      $values = array();
      foreach ($value[LANGUAGE_NONE] as $key => $link) {
        $values[]['value'] = $link['url'];
      }
      return $values;
    }
  }
  ...
  function convert_from_ax_single($value) {
    return array(LANGUAGE_NONE => array(0 => array('url' => $value)));
  }
