Synchronise Drupal users with Oempro lists and allow users to subscribe,
unsubscribe, and update member information. This module requires the
[Entity module](http://www.drupal.org/project/entity).

## Installation

1. Enable the Oempro Lists module and the Entity Module

2. To use Oempro Lists module, you will need to install and enable the Entity
API module [http://drupal.org/project/entity]([http://drupal.org/project/entity)

3. If you haven't done so already, add a list in your Oempro instance. Follow
these directions provided by Octeth on how to
[Setting up your first mailing list](http://docs.octeth.com/docs/oempro/users_quickstart/setting_up_your_first_mailing_list/)

4. Direct your browser to: http://example.com/admin/config/services/oempro
You will now see a "Lists and Users" tab. http://example.com/admin/config/services/oempro/lists
...
Required lists are automatically synchronised with the sites users. You can
...
* Enable Oempro webhooks for this list
When a user unsubscribes from a list or updates their profile outside of Drupal,
Oempro will trigger an event to update the user's cached Oempro member
...
another website, Oempro site directly, etc. will be trigger an update of the
cached member information within Drupal. This cached data means Drupal doesn't
have to contact Oempro every time it wants to determine a user's status, get
...
New subscribers will be sent a link with an email from Oempro that they must
...
Oempro lists to which they belong.

* Webhooks, see above.

Creating an Optional list will provide you with a block called Oempro
...
Creating an Free From list will provide you with a block called Oempro
Subscription Form: [list title]. This block contains a signup form with all
Oempro merge fields displayed.
