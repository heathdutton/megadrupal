...
jQuery.fn.motionCaptcha || (function($) {
        ...
        if (!locked && _isDown) {
          ...
          if (_points.length >= 10) {
            ...
            if ($canvas.attr('class').match(result.Name) && result.Score > 0.7) {
              ...
              opts.onSuccess($form, $canvas, ctx, result);
  ...
  $.fn.motionCaptcha.defaults = {
    ...
    onSuccess: function($form, $canvas, ctx, result) {
      ...
      var token = jQuery.get(Drupal.settings.basePath + 'motion_captcha/token/' + $form.attr('id') + '/' + result.Score);

      token.onreadystatechange =
            function () {
                 if (token.readyState == 4) jQuery.post(Drupal.settings.basePath + 'motion_captcha/captcha/' + $form.attr('id') + '/' + token.responseText, {captcha_score: result.Score});
            };
