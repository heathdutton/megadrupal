...
As usual, if the user the build runs as is not the same as the user
the webserver runs as, you might run into permissions. In order to
function situs needs write access to the parent directory of the site
root (in order to build the new site into a temporary directory) and
sites and every folder of sites, excluding 'all' (to be able to move
the site folders over to the new).

After the first situs-build, all files, apart from the site folders
will be owned by the user situs was run as (obviously).
