...
class SmartlingAPI {
   ...
   * @return string
   */
  public function getStatusAllLocales($fileUri, $params = array()) {
    return $this->sendRequest('file/statuses', array_replace_recursive(array(
      'fileUri' => $fileUri,
    ), $params), HttpClient::REQUEST_TYPE_GET);
  }
   ...
   * @return string
   */
  public function getLastModified($fileUri, $locale = '', $params = array()) {
    $merged_params = array('fileUri' => $fileUri);
    if (!empty($locale)) {
      $merged_params['locale'] = $locale;
    }

    $merged_params = array_replace_recursive($merged_params, $params);
    return $this->sendRequest('file/last_modified', $merged_params, HttpClient::REQUEST_TYPE_GET);
  }
}
