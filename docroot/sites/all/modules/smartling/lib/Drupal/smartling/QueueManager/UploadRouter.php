...
class UploadRouter {
  ...
  public function routeUploadRequest($entity_type, $entity, $languages, $async_mode = NULL) {
    if (entity_language($entity_type, $entity) !== language_default('language')) {
      $this->log->info('Entity is not in the site default language and cannot be submitted for translation. Type - @entity, Entity - @entity',
        array('@entity_type' => $entity_type, '@entity' => print_r($entity, TRUE)));
      return array('status' => 0, 'message' => 'Entity in non-default language');
    }
