...
USAGE
------
 * As most of the site(s), they are having user inputted comments(s) 
   by their user, again to evaluate those value(s), admin/other permitted
   role(s) will check the comment(s) and then post in their site(s).
 * After enabling this module, there is no need to check manually for 
   the sentiment description of user inputted comment(s).
