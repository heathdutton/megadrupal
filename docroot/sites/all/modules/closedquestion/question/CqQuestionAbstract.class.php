...
abstract class CqQuestionAbstract implements CqQuestionInterface {
   ...
   */
  public $supportedwidgets = array();

  /**
   * List of widget names found embedded in question
   */
  protected $widgetsfound = array();
   ...
   */
  public function handleNodeMathEval(&$node) {
    $expression = array();
   ...
   */
  public function handleNodeMathImport(&$node) {
    $expression = array();
    ...
    if ($item1 === NULL || $item2 === NULL || $item3 === NULL) {
      drupal_set_message(t('Attributes "varname", "importname" and "importnode" required for mathimport tags'));
      return;
    }
   ...
   */
  public function handleNodeTextlabel(&$node) {
   ...
   */
  public function handleNodeWidgetinfo(&$node) {
    $type = $node->getAttribute('type');
    $widgedId = $node->getAttribute('id');
    if (!is_null($type) && !is_null($widgedId) && array_key_exists($type, $this->supportedwidgets)) {
      ...
      case 'widget':
        ...
        if ($item !== NULL) {
          ...
          if (array_key_exists($widgetid, $this->widgetsfound)) {
            ...
            drupal_set_message(t('No matching widgetinfo found for widget with id: "' . $widgetid . '"'), 'warning');
          }
        }
        break;
    }
