...
# Download Drupal
cd /var/www 
git clone --recursive --branch 6.x http://git.drupal.org/project/drupal.git
chown www-data:www-data drupal -R
cd drupal

# Install Drupal
drush -y site-install default --db-url=mysql://root:@localhost/drupal-up --site-name=Drupal-up --sites-subdir=$fqdn
cd sites/$fqdn
