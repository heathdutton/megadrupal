...
class ARC2_StoreDeleteQueryHandler extends ARC2_StoreQueryHandler {
  ...
  function runQuery($infos) {
    ...
    /* graph(s) only */
    if (!$this->v('osf_triples', array(), $this->infos['query'])) {
  ...
  function deleteTriples() {
    ...
    $gq = $gq ? ' AND G.g IN (' . $gq . ')' : '';
    /* triples */
    foreach ($this->infos['query']['osf_triples'] as $t) {
      $q = '';
      $skip = 0;
  ...
  function deleteConstructedGraph() {
    ...
    $tgs = $this->infos['query']['target_graphs'];
    $this->infos = array('query' => array('osf_triples' => $triples, 'target_graphs' => $tgs));
