...
function save_task_success(r) {
  (function($) {
    ...
    // Update the task title area which can contain information about the
    // task regen settings. Initially the returned HTML will
    // contain a default color which we will reset back to the original via the classes
    original_classes = $("#task_title" + r.task_id).attr('class');
    $("#task_title" + r.task_id).parent().html(r.task_title);
    default_classes = $("#task_title" + r.task_id).attr('class');
    $("#task_title" + r.task_id).removeClass(default_classes);
    $("#task_title" + r.task_id).addClass(original_classes);
