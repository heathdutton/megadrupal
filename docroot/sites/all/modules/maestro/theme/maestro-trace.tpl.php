...
<?php
    ...
    foreach ($wf_res as $rec) {
      print l($rec->template_name, "maestro/trace/{$rec->tracking_id}/{$rec->initiating_pid}/0/{$sec_token}") . '&nbsp;&nbsp;';
    }
?>
  </div>
...
?>
              ...
              <tr>
                <td colspan="5" style="text-align: center;"><input class="form-submit" type="submit" value="<?php print t('Save All Task Changes'); ?>"></td>
              </tr>
            </tbody>
          </table>
        </div>
...
?>
  ...
  <script type="text/javascript">
    var ajax_url = '<?php print "{$ajax_url}/trace/{$properties->tracking_id}/{$properties->initiating_pid}/{$properties->queue_id}/" . drupal_get_token('maestro_user'); ?>';
  </script>
