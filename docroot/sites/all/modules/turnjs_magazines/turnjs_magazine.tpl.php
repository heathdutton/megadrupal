        ...
        if (!empty($res_images)) {
          ...
          foreach ($res_images as $idx => $res_image) {
            ...
            if (!empty($res_image['#item'])) {
              ...
              $image_array = array('path' => $full_image_path, 'attributes' => array());
              if (isset($res_image['#item']['alt'])) {
                $image_array['alt'] = $res_image['#item']['alt'];
              }
              if (isset($res_image['#item']['title'])) {
                $image_array['title'] = $res_image['#item']['title'];
              }
              if (isset($res_image['#item']['height'])) {
                $image_array['height'] = $res_image['#item']['height'];
              }
              if (isset($res_image['#item']['width'])) {
                $image_array['width'] = $res_image['#item']['width'];
              }
