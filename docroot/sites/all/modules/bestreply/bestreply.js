...
BestReply.moveToTop = function (){
  ...
  var onthispage = Drupal.settings.bestreply_onthispage;
  if(movetotop){
    if( onthispage ){ // it's on the same page
      ...
      .addClass('brclone')
      .slideDown('slow');
      // remove the class from the original comment
      $('.bestreply').addClass('brclosed').removeClass('bestreply');

      $(clone).addClass('bestreply').removeClass('brclosed'); // and give class "bestreply" to the clone.
      if(collapse){
        $('.brclosed .links').hide();
