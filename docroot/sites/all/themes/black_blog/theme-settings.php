...
*/
function black_blog_form_system_theme_settings_alter(&$form, $form_state) {
  ...
  $banners = black_blog_get_banners();
  print_r($banners);
  ...
  foreach ($banners as $image_data) {
    $form['banner']['images'][$i] = array(
      ...
      'image' => _black_blog_banner_form($image_data),
    );
  ...
  $form['banner']['image_upload'] = array(
   ...
   '#default_value' => variable_get('theme_black_blog_color_settings', 'default'),
    '#description' => t('Select the color you want to set.'),
  );
  $form['#submit'][]   = 'black_blog_settings_submit';
  $form['#submit'][]   = 'black_blog_switch_style_submit';
  return $form;
}
 ...
 * Save settings data.
 */
function black_blog_settings_submit($form, &$form_state) {
  $settings = array();
  ...
  if ($file = file_save_upload('image_upload')) {
    ...
    if ($image = _black_blog_save_image($file)) {
  ...
  // Save settings
  black_blog_set_banners($settings);
}

/**
 * Provvide default installation settings for marinelli.
 */
function _black_blog_install() {
  // Deafault data
  ...
  $src_base_path = drupal_get_path('theme', 'black_blog');
  ...
  foreach ($default_banners as $i => $data) {
    ...
    $banner = _black_blog_save_image($file);
  ...
  // Save banner data
  black_blog_set_banners($banners);

  // Flag theme is installed
  variable_set('theme_black_blog_first_install', FALSE);
}
 ...
 *    FALSE on error.
 */
function _black_blog_save_image($file, $banner_folder = 'public://blackblog/', $banner_thumb_folder = 'public://blackblog/thumb/') {
  // Check directory and create it (if not exist)
  _black_blog_check_dir($banner_folder);
  _black_blog_check_dir($banner_thumb_folder);
 ...
 *    Folder to check
 */
function _black_blog_check_dir($dir) {
 ...
 */
function _black_blog_banner_form($image_data) {
  $img_form = array();
...
/**
 *Submit handler for switching style
 */
function black_blog_switch_style_submit($form, &$form_state) {
 $color= $form_state['values']['switch_style'];
 variable_set('theme_black_blog_color_settings', $color);
