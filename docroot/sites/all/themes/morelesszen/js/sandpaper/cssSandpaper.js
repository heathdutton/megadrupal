...
var cssSandpaper = new function(){
    ...
    function fixOpacity(){
        ...
        for (var i in transformRules) {
            ...
            jQuery(rule.selector).each(function() {
							me.setOpacity(this, rule.value);
						});
        }
    ...
    function fixTransforms(){
        ...
        for (var i in transformRules) {
            var rule = transformRules[i];
            jQuery(rule.selector).each(function() {
							me.setTransform(this, rule.value);
						});
        }
    ...
    function fixBoxShadow(){
        ...
        for (var i in transformRules) {
            var rule = transformRules[i];
            jQuery(rule.selector).each(function() {
							me.setBoxShadow(this, rule.value);
						});
        }
    ...
    function fixLinearGradients(){
        ...
        for (var i in backgroundRules) {
            var rule = backgroundRules[i];
						jQuery(rule.selector).each(function() {
                me.setGradient(this, rule.value)
            });
        }
    }
        ...
        for (var i in backgroundRules) {
            var rule = backgroundRules[i];
                ...
                if (rule.value.indexOf('rgba(') == 0) {
                    me.setRGBABackground(this, rule.value);
                } else if (rule.value.indexOf('hsla(') == 0 || rule.value.indexOf('hsl(') == 0) {
                	
                	me.setHSLABackground(this, rule.value);
                } 
            });
        }
    }
        ...
        for (var i in colorRules) {
            ...
            jQuery(rule.selector).each(function() {
                ...
                if (ruleMatch) {
                	...
                	me.setHSLColor(this, cssProperty, rule.value);
    ...
    function getStyleSheets(){
        ...
        styleNodes = jQuery('style, link[rel="stylesheet"]').each(function() {
					if (jQuery(this).hasClass('cssSandpaper')) {
						styleSheets.push(getStyleSheet(this));
					}
				});
    ...
    function setClasses(){
        ...
        for (var i = 0; i < properties.length; i++) {
            ...
            if (CSS3Helpers.supports(prop)) {
                jQuery(htmlNode).addClass('cssSandpaper-' + prop);
            }
        }
		
		// Now .. remove the initially hidden classes
		var hiddenNodes = jQuery('.cssSandpaper-initiallyHidden').removeClass('cssSandpaper-initiallyHidden');
...
var CSS3Helpers = new function(){
    ...
    function hasIETransformWorkaround(obj){
        ...
        return jQuery(obj.parentNode).hasClass('IETransformContainer');
    }
    ...
    function addIETransformWorkaround(obj){
        if (!hasIETransformWorkaround(obj)) {
            ...
            jQuery(container).addClass('IETransformContainer');
...
jQuery(document).ready(function() {
	jQuery('body').once('cssSandpaper', function() { cssSandpaper.init() });
});
