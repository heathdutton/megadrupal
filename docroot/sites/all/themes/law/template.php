...
// Add Zen Tabs styles
if (theme_get_setting('law_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'law') . '/css/tabs.css');
}
/**
 * implement law_preprocess_html().
 */
function law_preprocess_html(&$vars) {
  ...
  foreach ($user->roles as $role) {
    $vars['classes_array'][] = 'role-' . law_id_safe($role);
  }
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = law_id_safe('page-' . $path);
    $vars['classes_array'][] = law_id_safe('section-' . $section);
  ...
  // Add IE classes.
  if (theme_get_setting('law_ie_enabled')) {
    $law_ie_enabled_versions = theme_get_setting('law_ie_enabled_versions');
    if (in_array('ie8', $law_ie_enabled_versions, TRUE)) {
...
/**
 * implements law_preprocess_page().
 */
function law_preprocess_page(&$vars, $hook) {
...
/**
 * implements law_preprocess_node().
 */
function law_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from law_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
/**
 * implements law_preprocess_block().
 */
function law_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function law_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('law_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('law_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('law_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('law_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('law_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 *
 */
function law_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = law_id_safe($element['#title']);
 ...
 */
function law_preprocess_menu_local_task(&$variables) {
 ...
 */
function law_menu_local_tasks(&$variables) {
  $output = '';
 ...
 */
function law_process_html(&$variables) {
 ...
 */
function law_process_page(&$variables) {
  // Hook into color.module.
  if (module_exists('color')) {
 ...
 */
function law_build_columns($columns) {
  $styles = array();
  $num_columns = 0;
  $first = -1;
