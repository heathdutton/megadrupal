...
// Add variables and paths needed for HTML5 and responsive support.
  ...
  $vars['path_to_black_hole'] = drupal_get_path('theme', 'black_hole');
...
$blockicons = theme_get_setting('blockicons');
  ...
  if ($blockicons == '3'){
    $vars['classes_array'][] = 'bni32';
  }
  if ($blockicons == '4'){
    $vars['classes_array'][] = 'bni48';
  }
...
// Set grids responsive stylesheets
  drupal_add_css(drupal_get_path('theme','black_hole').'/css/drupal/drupal-system-min.css', array('group' => CSS_SYSTEM, 'every_page' => TRUE, 'weight' => -1));
  drupal_add_css(drupal_get_path('theme','black_hole').'/css/drupal/drupal-default-min.css', array('group' => CSS_DEFAULT, 'every_page' => TRUE, 'weight' => -1));
  ...
  if(theme_get_setting('css_zone')) {
    ...
    drupal_add_css(drupal_get_path('theme','black_hole').'/css/yui/pure-min.css', array('group' => CSS_THEME, 'every_page' => TRUE, 'weight' => -2));
  }
  if(theme_get_setting('grid_responsive') == '1') {
    if(theme_get_setting('css_zone')) {
      ...
      drupal_add_css(drupal_get_path('theme','black_hole').'/css/yui/grids-responsive-old-ie-min.css', array('group' => CSS_THEME, 'browsers' => array('IE' => 'IE 8', '!IE' => FALSE), 'every_page' => TRUE, 'weight' => -1));
      drupal_add_css(drupal_get_path('theme','black_hole').'/css/yui/grids-responsive-min.css', array('group' => CSS_THEME, 'browsers' => array('IE' => 'gt IE 8'), 'every_page' => TRUE, 'weight' => -1));
    }
  }
global $language;
...
if(theme_get_setting('headerimg')) {
  ...
  if($lang_dir == 'rtl') {
    drupal_add_css(drupal_get_path('theme','black_hole').'/_custom/headerimg-rtl/rotate.php', array('group' => CSS_THEME, 'every_page' => TRUE, 'weight' => 2, 'preprocess' => FALSE));
  } else {
    drupal_add_css(drupal_get_path('theme','black_hole').'/_custom/headerimg/rotate.php', array('group' => CSS_THEME, 'every_page' => TRUE, 'weight' => 2, 'preprocess' => FALSE));
  }
}
  drupal_add_css(drupal_get_path('theme','black_hole').'/css/style-bh.css', array('group' => CSS_THEME, 'every_page' => TRUE, 'weight' => 3));
  drupal_add_css(drupal_get_path('theme','black_hole').'/css/'.get_black_hole_style().'.css', array('group' => CSS_THEME, 'every_page' => TRUE, 'weight' => 4));
  drupal_add_css(drupal_get_path('theme','black_hole').'/_custom/custom-style.css', array('group' => CSS_THEME, 'every_page' => TRUE, 'weight' => 6));
  drupal_add_css(drupal_get_path('theme','black_hole').'/css/print.css', array('group' => CSS_THEME, 'media' => 'print', 'every_page' => TRUE, 'weight' => 7));
...
if (theme_get_setting('slideshow_display')) {
  if (drupal_is_front_page() || theme_get_setting('slideshow_all')) {
    drupal_add_js(drupal_get_path('theme', 'black_hole') . '/js/jquery.flexslider-min.js', array('group' => JS_THEME));
    drupal_add_js(drupal_get_path('theme', 'black_hole') . '/js/slide.js', array('group' => JS_THEME));
    drupal_add_css(drupal_get_path('theme','black_hole').'/css/slider.css', array('group' => CSS_THEME, 'every_page' => TRUE, 'weight' => 5));
  }
}
 ...
 * HTML processing
 */
function black_hole_process_html(&$vars) {
 ...
 */
function black_hole_image($vars) {
...
function black_hole_button($vars) {
  $element = $vars ['element'];
...
function black_hole_image_button($vars) {
  $element = $vars ['element'];
 ...
 * Theme's pager
 */
function black_hole_item_list($vars){
...
function black_hole_pager_link($vars){
 ...
 */
/*
function black_hole_menu_tree($vars) {
...
*/
function black_hole_links__system_main_menu($vars, $is_child=false){
  ...
  foreach($vars['links'] as $link){
    ...
    if(empty($link['#title']))
      ...
      if(!empty($link['#below'])){
        ...
        if(theme_get_setting('dropdown') == '1') {
          $html .= black_hole_links__system_main_menu(array('links' => $link['#below']), true);
        }
        $html .= '</li>';
      }
      else
...
function black_hole_css_alter(&$css) {
  $exclude = array(
  // drupal-system.css
