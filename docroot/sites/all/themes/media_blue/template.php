<?php
function media_blue_form_alter(&$form, &$form_state, $form_id) {
...
function media_blue_preprocess_comment(&$vars) {
	$theme_path = drupal_get_path('theme', variable_get('theme_default', NULL));
  $vars['submitted'] = $vars['created'] ;
   $vars['default_photo'] = '<img src="' . base_path( ) . $theme_path . '/images/user.png" />';
}
function media_blue_preprocess_page(&$vars) {
  ...
  if (variable_get('theme_media_blue_first_install', TRUE)) {
    include_once('theme-settings.php');
    _media_blue_install();
  }

  //to print the banners
  $banners = media_blue_show_banners();
  $vars['banner'] = $banners;
 ...
 */
function media_blue_show_banners() {
  $banners = media_blue_get_banners(FALSE);
 ...
 */
function media_blue_get_banners($all = TRUE) {
  // Get all banners
  $banners = variable_get('theme_media_blue_banner_settings', array());
 ...
 *    Settings to save
 */
function media_blue_set_banners($value) {
  variable_set('theme_media_blue_banner_settings', $value);
}
