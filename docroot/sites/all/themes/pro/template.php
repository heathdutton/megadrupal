...
// Add Zen Tabs styles
if (theme_get_setting('pro_tabs')) {
  drupal_add_css( drupal_get_path('theme' , 'pro') . '/css/tabs.css');
}
/**
 *  Implements pro_preprocess_html()
 */
function pro_preprocess_html(&$vars)  {
  global $user;

  //Add role name classes (to allow css based show for admin/hidden from user)
  foreach ($user->roles as $role) {
    $vars['classes_array'][] = 'role-' . pro_id_safe($role);
  }
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = pro_id_safe('page-' . $path);
    $vars['classes_array'][] = pro_id_safe('section-' . $section);
  ...
  // Add IE classes.
  if (theme_get_setting('pro_ie_enabled')) {
    $pro_ie_enabled_versions = theme_get_setting('pro_ie_enabled_versions');
    if (in_array('ie8', $pro_ie_enabled_versions, TRUE)) {
...
/**
 *  Implements pro_preprocess_node()
 */
function pro_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from pro_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
/**
 *  Implements pro_preprocess_block()
 */
function pro_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function pro_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('pro_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('pro_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('pro_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('pro_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('pro_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 *
 */
function pro_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = pro_id_safe($element['#title']);
 ...
 */
function pro_preprocess_menu_local_task(&$variables) {
 ...
 */
function pro_menu_local_tasks(&$variables) {
  $output = '';
 ...
 */
function pro_process_html(&$variables) {
 ...
 */
function pro_process_page(&$variables) {
  // Hook into color.module.
  if (module_exists('color')) {
