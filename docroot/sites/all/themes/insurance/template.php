...
// Add Zen Tabs styles
if (theme_get_setting('insurance_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'insurance') .'/css/tabs.css');
}

function insurance_preprocess_html(&$vars) {
  global $user;

  //Add role name classes (to allow css based show for admin/hidden from user)
  foreach ($user->roles as $role){
    $vars['classes_array'][] = 'role-' . insurance_id_safe($role);
  }
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = insurance_id_safe('page-'. $path);
    $vars['classes_array'][] = insurance_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('insurance_ie_enabled')) {
    $insurance_ie_enabled_versions = theme_get_setting('insurance_ie_enabled_versions');
    if (in_array('ie8', $insurance_ie_enabled_versions, TRUE)) {
...
function insurance_preprocess_page(&$vars, $hook) {
...
function insurance_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from insurance_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function insurance_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function insurance_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('insurance_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('insurance_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('insurance_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('insurance_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('insurance_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function insurance_id_safe($string) {
 ...
 *
 */
function insurance_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = insurance_id_safe($element['#title']);
 ...
 */
function insurance_preprocess_menu_local_task(&$variables) {
 ...
 */
function insurance_menu_local_tasks(&$variables) {
  $output = '';
 ...
 */
function insurance_process_html(&$variables) {
 ...
 */
function insurance_process_page(&$variables) {
  // Hook into color.module.
  if (module_exists('color')) {
 ...
 */
function insurance_build_columns($columns) {
  $styles = array();
  $num_columns = 0;
  $first = -1;
