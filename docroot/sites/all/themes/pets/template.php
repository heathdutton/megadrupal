...
// Add Zen Tabs styles
if (theme_get_setting('pets_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'pets') . '/css/tabs.css');
}
/**
 * implements pets_preprocess_html().
 */
function pets_preprocess_html(&$vars) {
  global $user;

  //Add role name classes (to allow css based show for admin/hidden from user)
  foreach ($user->roles as $role) {
    $vars['classes_array'][] = 'role-' . pets_id_safe($role);
  }

  if (!$vars['is_front']) {
    // Add unique classes for each page and website pets
    $path = drupal_get_path_alias($_GET['q']);
    list($pets, ) = explode('/', $path, 2);
    $vars['classes_array'][] = 'with-subnav';
    $vars['classes_array'][] = pets_id_safe('page-' . $path);
    $vars['classes_array'][] = pets_id_safe('pets-' . $pets);
    ...
    if (arg(0) == 'node') {
      if (arg(1) == 'add') {
        ...
        // Add 'pets-node-add'
        $vars['classes_array'][] = 'pets-node-add';
      }
      elseif (is_numeric(arg(1)) && (arg(2) == 'edit' || arg(2) == 'delete')) {
        ...
        // Add 'pets-node-edit' or 'pets-node-delete'
        $vars['classes_array'][] ='pets-node-' . arg(2);
        }
        }
        }
  ...
  // Add IE classes.
  if (theme_get_setting('pets_ie_enabled')) {
    $pets_ie_enabled_versions = theme_get_setting('pets_ie_enabled_versions');
    if (in_array('ie8', $pets_ie_enabled_versions, TRUE)) {
...
/**
 * Implements pets_preprocess_page().
 */
function pets_preprocess_page(&$vars, $hook) {
...
/**
 * implements pets_preprocess_node().
 */
function pets_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from pets_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
/**
 * implements pets_preprocess_block().
 */
function pets_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function pets_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('pets_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('pets_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('pets_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('pets_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('pets_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 *
 */
function pets_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = pets_id_safe($element['#title']);
 ...
 */
function pets_preprocess_menu_local_task(&$variables) {
 ...
 */
function pets_menu_local_tasks(&$variables) {
  $output = '';
 ...
 */
function pets_process_html(&$variables) {
 ...
 */
function pets_process_page(&$variables) {
  // Hook into color.module.
  if (module_exists('color')) {
 ...
 */
function pets_build_columns($columns) {
  $styles = array();
  $num_columns = 0;
  $first = -1;
