...
/**
 * @file
 * Theme settings for the mojo theme.
 */

/**
 * Implements hook_form_system_theme_settings_alter().
 */
function mojo_form_system_theme_settings_alter(&$form, &$form_state, $form_id = NULL) {
  ...
  $recommended_modules = mojo_recommended_modules();
  ...
  if (!empty($recommended_modules)) {
    ...
    $form['recommended_modules']['hide_recommended_modules'] = array(
      ...
      '#ajax' => array(
        'callback' => 'mojo_ajax_settings_save',
      ),
      '#default_value' => $hide,
    );
 ...
 */
function mojo_ajax_settings_save($form, $form_state) {
  $theme = isset($form_state['build_info']['args'][0]) ? $form_state['build_info']['args'][0] : '';
 ...
 */
function mojo_recommended_modules() {
  $modules = array();
