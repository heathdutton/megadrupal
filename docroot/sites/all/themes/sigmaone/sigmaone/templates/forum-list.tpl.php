...
<table id="forum-<?php print $forum_id; ?>">
  ...
  <?php foreach ($forums as $child_id => $forum): ?>
    	<?php if ($forum->is_container) {
						$name_class = explode(' ', $forum->name);
						empty($name_class[0]) ? $child_class = 'show' : $child_class = 'forumChild';
						} ?>
   
  <?php if ($forum->is_container) { ?>
    <tr id="<?php print $name_class[0]; ?>" class="<?php print $forum->zebra; ?> open header">
  <?php } else { ?>
        <tr id="forum-list-<?php print $child_id; ?>" class="<?php print $forum->zebra; ?> <?php if (!empty($name_class[0])) print $name_class[0]; ?> <?php if (!empty($child_class)) print $child_class; ?>">
  <?php } ?>
      ...
      <td <?php print $forum->is_container ? 'colspan="4" class="container"': 'class="forum"'; ?>>
        ...
        <?php print str_repeat('<div class="indent">', $forum->depth); ?>
         <?php if ($forum->is_container) { ?>
          <div class="name"><a id="<?php print $name_class[0]; ?>" class="header" href="<?php print $forum->link; ?>"><?php print $forum->name; ?></a></div>
           <?php } else { ?>
