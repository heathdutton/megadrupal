...
*/
function corporate_blue_form_system_theme_settings_alter(&$form, $form_state) {
     $form['orbit'] = array(
    '#type' => 'fieldset',
  ...
  $form['banner'] = array(
    '#type' => 'fieldset',
    '#title' => t('Banner managment [Upload images with dimension 960*310]'),
    '#weight' => 1,
    '#collapsible' => TRUE,
    '#collapsed' => FALSE,
  ); 
  ...
  $banners = corporate_blue_get_banners();
  ...
  foreach ($banners as $image_data) {
    $form['banner']['images'][$i] = array(
      ...
      'image' => _corporate_blue_banner_form($image_data),
    );
  ...
  $form['#submit'][]   = 'corporate_blue_settings_submit';
  return $form;
}
 ...
 * Save settings data.
 */
function corporate_blue_settings_submit($form, &$form_state) {
  $settings = array();
  ...
  if ($file = file_save_upload('image_upload')) {
    ...
    if ($image = _corporate_blue_save_image($file)) {
  ...
  // Save settings
  corporate_blue_set_banners($settings);
}

/**
 * Provvide default installation settings for marinelli.
 */
function _corporate_blue_install() {
  // Deafault data
  $file = new stdClass;
  $banners = array();
  // Source base for images

  $src_base_path = drupal_get_path('theme', 'corporate_blue');
  ...
  foreach ($default_banners as $i => $data) {
    ...
    $banner = _corporate_blue_save_image($file);
  ...
  // Save banner data
  corporate_blue_set_banners($banners);

  // Flag theme is installed
  variable_set('theme_corporate_blue_first_install', FALSE);
}
 ...
 *    FALSE on error.
 */
function _corporate_blue_save_image($file, $banner_folder = 'public://corporateblue/', $banner_thumb_folder = 'public://corporateblue/thumb/') {
  // Check directory and create it (if not exist)
  _corporate_blue_check_dir($banner_folder);
  _corporate_blue_check_dir($banner_thumb_folder);
 ...
 *    Folder to check
 */
function _corporate_blue_check_dir($dir) {
 ...
 */
function _corporate_blue_banner_form($image_data) {
  $img_form = array();
