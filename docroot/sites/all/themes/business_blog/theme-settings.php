...
*/
function business_blog_form_system_theme_settings_alter(&$form, $form_state) {
  ...
  $banners = business_blog_get_banners();
  ...
  foreach ($banners as $image_data) {
    $form['banner']['images'][$i] = array(
      ...
      'image' => _business_blog_banner_form($image_data),
    );
  ...
  $form['#submit'][]   = 'business_blog_settings_submit';
  return $form;
}
 ...
 * Save settings data.
 */
function business_blog_settings_submit($form, &$form_state) {
  $settings = array();
  ...
  if ($file = file_save_upload('image_upload')) {
    ...
    if ($image = _business_blog_save_image($file)) {
  ...
  // Save settings
  business_blog_set_banners($settings);
}

/**
 * Provvide default installation settings for marinelli.
 */
function _business_blog_install() {
  // Deafault data
  $file = new stdClass;
  $banners = array();
  // Source base for images

  $src_base_path = drupal_get_path('theme', 'business_blog');
  ...
  foreach ($default_banners as $i => $data) {
    ...
    $banner = _business_blog_save_image($file);
  ...
  // Save banner data
  business_blog_set_banners($banners);

  // Flag theme is installed
  variable_set('theme_business_blog_first_install', FALSE);
}
 ...
 *    FALSE on error.
 */
function _business_blog_save_image($file, $banner_folder = 'public://business/', $banner_thumb_folder = 'public://business/thumb/') {
  // Check directory and create it (if not exist)
  _business_blog_check_dir($banner_folder);
  _business_blog_check_dir($banner_thumb_folder);
 ...
 *    Folder to check
 */
function _business_blog_check_dir($dir) {
 ...
 */
function _business_blog_banner_form($image_data) {
  $img_form = array();
