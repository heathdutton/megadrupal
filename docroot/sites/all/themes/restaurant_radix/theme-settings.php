...
function restaurant_radix_form_system_theme_settings_alter(&$form, &$form_state) {
  ...
  $form_state['build_info']['files']['form'] = drupal_get_path('theme', 'restaurant_radix') . '/theme-settings.php';
...
//  $form['branding']['primary_color'] = array(
//    '#type' => 'jquery_colorpicker',
//    '#title' => t('Primary color'),
//    '#default_value' => theme_get_setting('primary_color'),
//  );
//
//  $form['branding']['secondary_color'] = array(
//    '#type' => 'jquery_colorpicker',
...
//  );
  ...
  $form['#submit'][] = 'restaurant_radix_form_system_theme_settings_submit';
 ...
 */
function restaurant_radix_form_system_theme_settings_submit($form, &$form_state) {
  ...
  foreach ($values as $name => $value) {
    if (preg_match('/_image$/', $name)) {
      if (!empty($values[$name])) {
        ...
        file_save($file);
        file_usage_add($file, 'restaurant_radix', 'theme', 1);
